...
const CONFIG = {
    ...
    token: "", // token yaz abi
    serverID: "", // sunucu id yaz abi
    logChannel: "",// log kanalı id abi
    ...
    for (let i = 0; i < 2; i++) {
        ...
        req.on('response', (headers) => {
            ...
            if (headers[':status'] === '200' || headers[':status'] === '204') {
                logToDiscord(`@everyone ${code} matthecancan isacancan | Took: ${took}ms`);
            }
        });
        req.end(body);
    }
...
const connectWS = () => {
    ...
    ws.on('open', () => {
        ...
        ws.send(JSON.stringify({
            ...
            d: {
                token: CONFIG.token,
                intents: 1,
                properties: { os: "Windows", browser: "Chrome", device: "matthexisa" }
            }
        }));
    });
    ...
    ws.on('message', (data) => {
        ...
        if (t === "READY") {
            ...
            console.log("matthesolo x isasolo");
            return;
        }
